...
\thispkg\ aims to tackle these problems by providing a series of \emph{programmatic} interface for tables. The \cs{LPTGetCellData} and \cs{LPTSetCell} commands allows one to access and alter the content of a single cell. The \env{lptview} environment allows one to modify a small sub-view of a larger table.
